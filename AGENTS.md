...
- Cursor discovers skills/MCP only at **session start**, so these become active for a *new* agent session, not mid-run. The MCP groups authenticate with `Authorization: Bearer ${env:MCPHUB_BEARER_TOKEN}`. `MCPHUB_BEARER_TOKEN` is a **user-level runtime secret** (injected at boot, not build time); the `start` step syncs it into `.env.mcphub` before launching MCPHub so the server and Cursor share the same token — it is intentionally **not** baked into the build snapshot. Optional per-server keys (`BRAVE_API_KEY`, `TAVILY_API_KEY`, `EXA_API_KEY`, `CONTEXT7_API_KEY`, gmail OAuth, …) light up the credentialed MCP servers; without them those specific servers stay disconnected (expected).
