...
- [x] **TASK-043** Fail closed with `unknown_tool` for catalog misses, `event_unavailable` when a cataloged event cannot be dispatched, and `unhandled_tool` for catalog entries without a supported local action.
...
- [x] **TASK-048** Run `pnpm test`, `python3 scripts/validate_planning.py`, `python3 scripts/validate_planning.py --runtime`, and the forbidden-endpoint ripgrep review. Replace all Wave 4 TBD placeholders only with executed results.
- [x] **TASK-049** Update the Wave 4 evidence table and recommendation after TASK-048. Do not claim PASS or add network-capable tools while validation evidence is TBD.
