...
- [x] **TASK-033** Forward only the local filename, Blob/bytes, and MIME type to the host; normalize host rejection and exceptions without exposing exception content.
...
- [ ] **TASK-038** Run `pnpm test`, `python3 scripts/validate_planning.py`, `python3 scripts/validate_planning.py --runtime`, and the forbidden-endpoint ripgrep review. Replace all Wave 3 TBD placeholders only with executed results.
- [ ] **TASK-039** Update the Wave 3 evidence table and recommendation after TASK-038. Do not claim a shipped native host or PASS while validation evidence is TBD.
